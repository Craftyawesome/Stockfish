...
Value Eval::evaluate(const Eval::NNUE::Networks& networks,
                     const Position&             pos,
                     int                         optimism,
                     bool                        higherThresholds) {

    assert(!pos.checkers());

    int  simpleEval = simple_eval(pos, pos.side_to_move());
    bool smallNet   = std::abs(simpleEval) > SmallNetThreshold + 1024 * higherThresholds;
    bool psqtOnly   = std::abs(simpleEval) > PsqtOnlyThreshold + 2048 * higherThresholds;
    int  nnueComplexity;
    int  v;
