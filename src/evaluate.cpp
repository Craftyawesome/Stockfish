...
Value Eval::evaluate(const Position& pos, int optimism, Depth depth) {
    ...
    bool lazy = std::abs(simpleEval) > 16 * depth + 2250;
    if (lazy)
        v = simpleEval;
    else
    {
        bool smallNet = std::abs(simpleEval) > 16 * depth + 750;
