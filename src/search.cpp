...
Value futility_margin(Depth d, bool noTtCutNode, bool improving, bool oppWorsening) {
    Value futilityMult       = 124 - 42 * noTtCutNode;
...
int stat_bonus(Depth d) { return std::clamp(207 * d - 288, 20, 1667); }

// History and stats update malus, based on depth
int stat_malus(Depth d) { return (d < 4 ? 800 * d - 284 : 1869); }
...
void Search::Worker::iterative_deepening() {
           ...
           && !(limits.depth && mainThread && rootDepth > limits.depth))
    {
        ...
        for (pvIdx = 0; pvIdx < multiPV; ++pvIdx)
        {
            ...
            delta     = 7 + avg * avg / 11420;
            alpha     = std::max(avg - delta, -VALUE_INFINITE);
            beta      = std::min(avg + delta, VALUE_INFINITE);

            // Adjust optimism based on root move's averageScore (~4 Elo)
            optimism[us]  = 110 * avg / (std::abs(avg) + 103);
...
void Search::Worker::clear() {
    ...
    for (size_t i = 1; i < reductions.size(); ++i)
        reductions[i] = int((19.53 + std::log(size_t(options["Threads"])) / 2) * std::log(i));
...
Value Search::Worker::search(
  Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    {
        int bonus = std::clamp(-10 * int((ss - 1)->staticEval + ss->staticEval), -1763, 1497) + 712;
    ...
    // return a fail low.
    if (eval < alpha - 571 - 260 * depth * depth)
    {
    ...
    if (!PvNode && (ss - 1)->currentMove != Move::null() && (ss - 1)->statScore < 12572
        && eval >= beta && ss->staticEval >= beta - 23 * depth + 392 && !excludedMove
    ...
    probCutBeta = beta + 181 - 54 * improving;
    if (
      !PvNode && depth > 3
...
moves_loop:  // When in check, search starts here
    ...
    while ((move = mp.next_move(moveCountPruning)) != Move::none())
    {
        ...
        if (!rootNode && pos.non_pawn_material(us) && bestValue > VALUE_TB_LOSS_IN_MAX_PLY)
        {
            ...
            if (capture || givesCheck)
            {
                ...
                {
                    Value futilityValue = ss->staticEval + 308 + 253 * lmrDepth
                ...
                int seeHist = std::clamp(captHist / 29, -165 * depth, 175 * depth);
            ...
            else
            {
                ...
                if (lmrDepth < 6 && history < -3747 * depth)
                    continue;

                history += 2 * thisThread->mainHistory[us][move.from_to()];

                lmrDepth += history / 4337;

                Value futilityValue =
                  ss->staticEval + (bestValue < ss->staticEval - 59 ? 169 : 57) + 117 * lmrDepth;
                ...
                if (!pos.see_ge(move, -21 * lmrDepth * lmrDepth))
                    continue;
            }
        }
        ...
        if (ss->ply < thisThread->rootDepth * 2)
        {
            ...
            {
                Value singularBeta  = ttData.value - (55 + 97 * (ss->ttPv && !PvNode)) * depth / 64;
                ...
                {
                    int doubleMargin = 336 * PvNode - 177 * !ttCapture;
                    int tripleMargin = 93 + 317 * PvNode - 183 * !ttCapture + 105 * ss->ttPv;
        ...
        ss->statScore = 2 * thisThread->mainHistory[us][move.from_to()]
                      + (*contHist[0])[movedPiece][move.to_sq()]
                      + (*contHist[1])[movedPiece][move.to_sq()] - 5977;

        // Decrease/increase reduction for moves with a good/bad history (~8 Elo)
        r -= ss->statScore / 11704;
    ...
    {
        int bonus = (99 * (depth > 5) + 120 * (PvNode || cutNode) + 146 * ((ss - 1)->moveCount > 8)
                     + 60 * (!ss->inCheck && bestValue <= ss->staticEval - 118)
                     + 149 * (!(ss - 1)->inCheck && bestValue <= -(ss - 1)->staticEval - 72));
...
Depth Search::Worker::reduction(bool i, Depth d, int mn, int delta) const {
    ...
    return (reductionScale + 1299 - delta * 657 / rootDelta) / 1024 + (!i && reductionScale > 1346);
}
