...
void Search::Worker::start_searching() {
    ...
    tt.biasedHashfull.fetch_sub(100, std::memory_order_relaxed);
...
Value Search::Worker::search(
  Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    bool     doubleThresholds = tt.biasedHashfull.load(std::memory_order_relaxed) > 700;
    ...
    if (!rootNode)
    {
        ...
        if (threads.stop.load(std::memory_order_relaxed) || pos.is_draw(ss->ply)
            ...
            return (ss->ply >= MAX_PLY && !ss->inCheck)
                   ? evaluate(networks, pos, thisThread->optimism[us], doubleThresholds)
    ...
    else if (excludedMove)
    {
        ...
        Eval::NNUE::hint_common_parent_position(pos, networks, doubleThresholds);
    ...
    {
        unadjustedStaticEval = evaluate(networks, pos, thisThread->optimism[us], doubleThresholds);
        ss->staticEval = eval = to_corrected_static_eval(unadjustedStaticEval, *thisThread, pos);
