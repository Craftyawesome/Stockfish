...
Value Search::Worker::search(
  Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    if (!rootNode)
    {
        ...
        if (threads.stop.load(std::memory_order_relaxed) || pos.is_draw(ss->ply)
            ...
            return (ss->ply >= MAX_PLY && !ss->inCheck)
                   ? evaluate(networks, pos, thisThread->optimism[us], options["Allow_Smallnet"])
    ...
    else if (excludedMove)
    {
        ...
        Eval::NNUE::hint_common_parent_position(pos, networks, options["Allow_Smallnet"]);
    ...
    else if (ss->ttHit)
    {
        ...
        if (unadjustedStaticEval == VALUE_NONE)
            unadjustedStaticEval = evaluate(networks, pos, thisThread->optimism[us], options["Allow_Smallnet"]);
        else if (PvNode)
            Eval::NNUE::hint_common_parent_position(pos, networks, options["Allow_Smallnet"]);
